...
```{r product line vs Rating}
ggplot(df, aes(x=Product.line, y=Rating)) +
  geom_point()
```

The categories are indistinguishable in terms of rating

```{r}
ggplot(df ,aes(Gender, Rating)) +
  geom_point()
```

Rating is equally distributed in terms of gender

```{r Payment}
ggplot(df, aes(Payment, Rating)) +
  geom_point()
```


```{r Gross Income vs Rating}
ggplot(df, aes(gross.income, Rating)) +
  geom_point()
```

A lot of shoppers don't earn as much as is evidenced by concentrated points on the lower side of Gross Income. They therefore contribute the most to rating their experiences but it should be noted that high income earners are still distributed around various ratings. The highest earners have a relatively negative experience. 

```{r Customer type vs Rating}
ggplot(df, aes(Customer.type , Rating)) +
  geom_point()
```

Members and non members have equally distributed experiences in the stores.

```{r Tax vs Rating}
ggplot(df, aes(Tax, Rating)) +
  geom_point()
```

There are no discernable facts from the scatter plot above.

```{r Unit Price vs Rating}
ggplot(df, aes(Unit.price, Rating)) +
  geom_point()
```

There is no immediate relationship between Unit Price of items and Ratings


```{r Total Price vs Rating}
ggplot(df, aes(Total, Rating)) +
  geom_point()
```

There is no direct relationship between Total price and rating.
...
```
The follwing variables show strong correlation:
